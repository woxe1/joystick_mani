...
Usage:
1) Copy this file and `freecad_udp_orbit_macro.py` into your FreeCAD Macros folder.
2) Add a small loader line to FreeCAD's `InitGui.py` to execute this file on startup.

This helper waits until a GUI document and active 3D view exist, then starts the
UDP orbit macro once. Re-running it is safe: an older pending watcher is stopped.
"""
...
def start_udp_orbit_autostart() -> None:
    ...
    # Try immediately too, in case FreeCAD already has an active view.
