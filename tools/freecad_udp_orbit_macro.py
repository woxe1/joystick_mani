...
How to use:
1) Run this macro inside FreeCAD (Macro -> Macros -> Execute).
2) Run `py tools/freecad_hid_udp_bridge.py` from terminal.
3) Keep focus in FreeCAD 3D view.
"""
...
def _pivot_from_selection_or_scene() -> coin.SbVec3f:
    # 1) Selected object center, if available.
...
def _apply_orbit(dx: float, dy: float) -> None:
    ...
    # Prevent singularity when camera is exactly at pivot.
    if rel.length() < 1e-6:
    ...
    # Dynamic up lets camera pass vertical poles around the object.
...
def _axis_lock(x: float, y: float) -> tuple[float, float]:
    ...
    # If one axis dominates strongly, suppress the other for crisp control.
...
def stop_udp_orbit() -> None:
    ...
    if _sock is not None:
        try:
            ...
            pass
        _sock = None
    # clear singleton state reference
    try:
...
def start_udp_orbit() -> None:
    ...
    # Stop previous running instance (from older macro execution context), if any.
